...
#R aborts during 2016
#Years <- c(2013,2015,2017,2018,2020)
#Crops <- c('Corn','Corn','Corn','Soybeans','Corn')
...
#methods=c('gam','te','tp','lm')

# methods <- methods[1:2]
# re c(2,9),

ranges = matrix(c(c(2,24),
                  c(2,24),
                  c(4,300),
                  c(2,24),
                  c(2,24),
...
print(methods[j])
  ...
  for(i in 1:length(Years)) {
    ...
    tmp$Year <- Years[i]
    SelectionG <- rbind(SelectionG,tmp$Data)
  
    tmp <- select.best.variogram(Models[[i]])
    tmp$Data$Year <- Years[i]
    BestBICG <- rbind(BestBICG,tmp$Data)
print(paste("Best BIC, K=",tmp$K))

    tmp <- select.best.variogram(Models[[i]],criteria='slope')
    tmp$Data$Year <- Years[i]
    BestSlopeG <- rbind(BestSlopeG,tmp$Data)
print(paste("Best Slope, K=",tmp$K))
    tmp <- select.best.variogram(Models[[i]],criteria='nugget')
    tmp$Data$Year <- Years[i]
    BestNuggetG <- rbind(BestNuggetG,tmp$Data)
print(paste("Best Nugget, K=",tmp$K))
  }
  #Rprof(NULL)
...
ggplot(pooled[pooled$Criteria=='BIC' & pooled$Source=='Model',], aes(Distance,Gamma)) + 
    ...
    geom_smooth(aes(group = Method,color=Method), se=FALSE) +
    scale_colour_manual(values=c(cbPalette)) + facet_wrap(~Year) +
    labs(title = "BIC Selection, Model Variograms")
```
