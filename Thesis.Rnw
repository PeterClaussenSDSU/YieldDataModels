...
\depthead{Kurt Cogswell}     % dept head name
...
#Example1.dat$Pass[Example1.dat$Heading>100] <- Example1.dat$Pass[Example1.dat$Heading>100] + 400
Example1.dat$PassNo <- Example1.dat$Pass

#Example1.dat$Block <- ceiling(Example1.dat$Pair/2)
#Example1.dat$Strip <- Example1.dat$Pair %/% Example1.dat$Block
#Example1.dat$Block <- as.factor(Example1.dat$Block)
Example1.dat$Pass <- as.factor(Example1.dat$Pass)
Example1.dat$Pair <- as.factor(Example1.dat$Pair)
#Example1.dat$Strip <- as.factor(Example1.dat$Strip)
@
...
My brother is a farmer, and participated in the SDSU on-farm strip trial program. He was supplied with a small amount of product (fungicide) to test on a portion of his soybean crop lands. The work presented in this thesis works toward a simple goal - can we determine with some degree of statistical rigor if the fungicide application was effective? The area of design and analysis of on-farm participatory research has not undergone the degree of development, with respect to experimental design and analysis, as the more traditional "small-plot" trials, which have been conducted worldwide in some form for more than a century. The material in this thesis is presented as a contribution toward the development of methods for analysis on-farm strip trials.

\chapter{On-farm strip trials and the problem of finding functional forms for strips}

\section{Introduction}

Collaborative research between agronomists and farmers has a long history \cite{doerge-07-1999, riley-03-2001, agriculture-2024}, but precision agriculture (PA) technologies have greatly increased opportunities for on-farm research (for example, \cite{lawes.r-03-2012, taylor-08-2011}) . Many types of on-farm participatory trials have been explored over the past several decades \cite{riley-03-2001, kyveryga-11-2019}. 
...
In practice, a pass as analyzed may be trimmed to exclude the end-of-row headlands, which may include overlapping applications \cite{bullock-11-2019}. Strips should also ideally be applied to inner rows only to avoid side of field effects \cite{bullock-11-2019}.
...
We recognize that there is inherent spatial variability in the accuracy of yield monitors \cite{gauci-12-2023} and that yield monitor data frequently contains artifacts (\cite{marchant-2019}) but an appropriate choice of interpolation of yield points can minimize point-to-point variability.


Strictly speaking, on-farm strip trials do no necessarily require the use of GPS enabled harvesters. In the past, weigh wagons have been used to measure yield of strips in on-farm participatory testing - \cite{wuest-1994, yan-03-2002, nelson-06-2015} - for example. However, this method of determine strip yield is not suitable to analysis as functional data; while such methods have been in common use and are valid in context, we will ignore such experiments for our proposed analysis. 


We also recognize that some on-farm experiment can be conducted as single strips; this method requires multiple strips. Other methods have been proposed for the analysis of single-strip trials \cite{cho-2021, hatfield-06-2018}. We recognize that on-farm strip trials may be unreplicated within a single location, but analyzed across multiple locations (\cite{yan-03-2002,schmidt-04-2018}). We also recognize that on-farm trials may be paired with research station-based traditional small-plot experiments, as with mother-baby trials \cite{virk-2008}.
...
Means model          & $y_{ij} = \mu_i + e_{ij}$ 
                     & $y_{ij}(x) = \mu_i(x) + e_{ij}(x)$  \\ 
Paired Difference    & $d_j = y_{1j} - y_{2j}$ 
                     & $d_j(x) = y_{1j}(x) - y_{2j}(x)$ \\
Mean Difference      & $\bar{d} = \frac{1}{n} \sum_{j=1}^n d_j$ 
                     & $\bar{d}(x) = \frac{1}{n(x)} \sum_{j=1}^n(x) d_j(x)$  \\ 
Number of Pairs      & $n$ 
                     & $n(x)$  \\ 
Standard deviation of paired differences    & $S_d = \sqrt{\frac{\sum_{j=1}^n (d_j - \bar{d})^2}{n-1}}$ 
                     & $S_d(x) = \sqrt{\frac{\sum_{j=1}^n(x) (d_j(x) - \bar{d(x)})^2}{n(x)-1}}$  \\
$t$ statistic        & $t_0 = \frac{\bar{d}}{S_d/\sqrt{n}}$ 
                     & $t_0(x) = \frac{\bar{d}(x)}{S_d(x)/\sqrt{n(x)}}$  \\ 
...
\section{Implementations of Smoothing Functions for Functional Data}
...
\pagebreak
\section{General Overview of Yield Monitor Data}\label{YieldMonitorOverview}

Yield is recorded in units of bushels per acre, but this requires the use of several sensors in the harvester \cite{whelan-2013, luck-2014}. Grain flow sensors measure the rate at which grain moves within the harvester. Typically, there are two types of sensors in used; impact-type which measures the force of grain hitting a metal plate connected to a strain gauge or potentiometer, and volumetric-type sensors which measure the volume of grain as it is lifted inside the harvester in an elevator. Flow rate is typically recorded at one second intervals as in our example data.
...
The example data are taken from the Example 1 (Section \ref{Example1Section}) data set, a split-planter variety trial. We take the first harvest pass of the data. The original yield observations for this harvest pass are shown in Figure \ref{RawSwath}
...
@
\caption{Representative harvest pass with no smoothing.}\label{RawSwath}
\end{figure}
...
@
\caption{Interpolation of yield points using a linear model of degree 12}\label{PolynomialSmoothing}
\end{figure}
...
@
\caption{Smoothing the raw data with polynomials of increasing order}\label{SeveralPolynomials}
\end{figure}
...
@
\caption{Piecewise linear interpolation}\label{PiecewiseLinear}
\end{figure}
...
This add as sum of squared second differences of the function at the knots; when the basis functions are splines this is represented by the second integral of the smoothing function,
\begin{equation}
...
The \texttt{mgcv} function using the \texttt{s(x, ...)} syntax to define a smoothing function over a covariate and \texttt{...} are additional arguments, such as \texttt{bs='bs'} that specify the basis functions and \texttt{k}, which specifies the dimension of the basis used to represent the smooth term \cite{wood-2017}.  
...
In Figure \ref{ComparingSplineFits}, we compare \texttt{ns} and \texttt{bs} fit to the single swath example and and equivalent b-spline fit using \texttt{mgcv} \texttt{gam}. We see that the \texttt{ns} fit with \texttt{df=12+1}, \texttt{bs} fit with \texttt{df=12+3} and \texttt{s(Easting,bs='bs',k=16)} product interpolating functions that are nearly identical except small differences at the edges. 
...
ggplot(ComparisonMGCV.dat, aes(Easting, Yield)) + 
   ...
   scale_colour_manual(values=cbPalette[1:11])
@
\caption{Different spline smoothers using \texttt{gam} defaults}\label{ComparingGAMSplines}
\end{figure}
...
ggplot(ComparisonBRM.dat, aes(Easting,Yield)) + 
   ...
   scale_colour_manual(values=cbPalette[1:11])
@
\caption{Different spline smoothers using \texttt{brm} defaults}\label{ComparingBRMSplines}
\end{figure}
...
\pagebreak
\subsection{Comparing different smoothing functions}
...
@
\caption{Ordinary cross validation for increasing number of knots specified for select smoothing functions}\label{SplitsCompOCV}
\end{figure}
...
Figure \ref{ComparingSSplines} shows the default smoothing functions \texttt{s} from the \texttt{mgcv}, \texttt{brm} and \texttt{gam} libraries. There is little difference between \texttt{mgcv} and \texttt{brm}, while the default smoother is much more "smooth" for the \texttt{gam} function. This is not suprising, given that the \texttt{brm} \texttt{s} function is largely a wrapper for the \texttt{mgcv} \texttt{s} function \cite{brms_s}. 
...
ggplot(Comparison2.dat, aes(Easting,Yield)) + 
   ...
   scale_colour_manual(values=cbPalette[1:11])
@
\caption{Different implementations of the \texttt{s()} specifier}\label{ComparingSSplines}
\end{figure}
...
@
\caption{Smoothing curves for a single harvest swath using \texttt{lm} with \texttt{ns, df = K+1}}\label{PredictionsNS}
\end{figure}
...
@
\caption{Smoothing curves for a single harvest swath using mgcv gam with the default smoother, k = Knots+4}\label{PredictionsGAM}
\end{figure}
...
@
\caption{Smoothing curves for a single harvest swath using \texttt{mgcv gam} with the \texttt{bs='bs'} smoother, \texttt{k = Knots+4}}\label{PredictionsGAMBS}
\end{figure}
...
ggplot(PredictionsGAMAD.dat, aes(Easting,Yield)) +
  ...
  geom_line(linewidth=1,aes(color=Knots, linetype=Knots)) + 
  scale_colour_manual(values=cbPalette[2:11]) + 
  labs(colour = "K", x="Easting", y="Yield", title = "Single Swath, mgcv ad smoothing")
@
\caption{Smoothing curves for a single harvest swath using \texttt{mgcv gam} with the \texttt{bs='ad'} smoother, \texttt{k = Knots+4}}\label{PredictionsGAMAD}
\end{figure}
...
@
\caption{Smoothing curves for a single harvest swath using \texttt{brm} with the default smoother, \texttt{k = Knots+4}}\label{PredictionsBRM}
\end{figure}
...
@
\caption{Smoothing curves for a single harvest pass using \texttt{fda} with \texttt{create.bspline.basis}}\label{PredictionsFDA}
\end{figure}
...
\chapter{Functional data analysis of example strip trials}
...
Units in GPS were converted to meters based on the haversine formula \cite{haversine}, using the R function 
...
\texttt{Easting} as the number of meters from the lower left (southwest) corner in an easterly direction of the cropland where the grain was harvested, `Northing` as the number of meters in a northerly direction, and `Yield`, which is yield in bushels per acre, renamed from the original data; this was done to make analysis consistent across yield data providers.

\pagebreak
\section{Example 1}\label{Example1Section}

In this experiment, two varieties were tested using a split-planter comparison method \cite{doerge-07-1999}.
...
@
\caption{Histogram of yield samples for Example 1}\label{Example1Histogram}
\end{figure}
...
\caption{Seeding and yield monitor harvest maps for Example 1}\label{Example1Maps}
\end{figure}
...
#compute min and max northing values so we can estimate devations from a straight line
...
Example1HarvestPasses.dat$Strip <- ceiling(Example1HarvestPasses.dat$Pass/2)
#aggregate by strips
...
@
\caption{Yields from Table \ref{SummaryExample1Strips} plotted by \texttt{Northing}}\label{Example1StripYieldsPlot}
\end{figure}
...
\begin{tabular}{r r} 
 ...
 6 & 15.42 \\
\end{tabular}
\caption{Differences used for paired $t$-test. Yield difference is calcuated by the yield for variety B minus yield for variety E, where yields are taken from Table \ref{SummaryExample1Strips}}
...
\subsection{Functional Data Analysis of Example 1}
...
labs(colour = "Variety", x="Easting", y="Yield", title = "Example 1 Harvest Passes") +
  ylim(80, 240)
@
\caption{Raw yield monitor estimates for the harvest strips in Example 1}\label{Example1RawHarvestPasses}
\end{figure}
...
labs(colour = "Variety", x="Easting", y="Yield", title = "Example 1 Smoothed Harvest Passes") +
  ylim(80, 240)
@
\caption{GAM smoothed harvest passes from Figure \ref{Example1RawHarvestPasses}}\label{Example1SmoothedHarvestPasses}
\end{figure}
...
labs(colour = "Variety", x="Easting", y="Yield", title = "Example 1 Strips") +
  ylim(80, 240)
@
\caption{Smoothed harvest passes pooled into experimental strips for Example 1}\label{Example1AnalyzedStrips}
\end{figure}
...
for(i in 1:length(Example1.basis)) {
  ...
  Differences <- MeansB$Yield - MeansE$Yield
  TTestExample1$Delta[i] <- mean(Differences)
  TTestExample1$SD[i] <- sd(Differences)
}
...
%  Signficant = rep(TTestExample1$P<0.05,4)
%)
...
@
\caption{Pointwise yield differences $\bar{d})(x)$ for Example 1}\label{Example1Differences}
\end{figure}
...
@
\caption{Pointwise yield differences standard deviations $S_d(x)$ for Example 1}\label{Example1SD}
\end{figure}
...
@
\caption{Pointwise $t(x)$ ratio values for Example 1}\label{Example1T}
\end{figure}
...
@
\caption{Pointwise $P(>t)$ values for Example 1}\label{Example1P}
\end{figure}
...
@
\caption{pointwise mean difference and confidence intervals among variety strips from Example 1}\label{CIPlotExample1}
\end{figure}
...
\pagebreak
\section{Example 2}\label{Example2Section}
...
@
\caption{Histogram of yield samples for Example 2, before and after cleaning}\label{Example2Histograms}
\end{figure}
...
\begin{figure}\centering
<<Example2Maps,fig=TRUE,echo=FALSE, width=8, height=6>>=
grid.arrange(
  arrangeGrob(
...
@
\caption{Spraying and yield monitor harvest maps for Example 2}\label{Example2SprayYieldMaps}
\end{figure}
...
@
\caption{Yield monitor harvest data from Example 2}\label{Example2ActualYield}
\end{figure}
...
for(pass in Example2Passes) {
  ...
  Strip2 <- c(Strip2,rep(current.pass$Strip[1],L))
  #Block2 <- c(Block2,rep(current.pass$Block[1],L))
  Pass2 <- c(Pass2,rep(current.pass$Pass[1],L))
  Treated2 <- c(Treated2,rep(current.pass$Sprayed[1],L))
...
@
\caption{Smoothed harvest strips for Example 2}\label{Example2HarvestPasses}
\end{figure}
...
@
\caption{Smoothed harvest strips pooled by by strip for Example 2}\label{Example2Strips}
\end{figure}
...
for(i in 1:length(Example2.basis)) {
  ...
  if(length(MeansSprayed$Yield) == length(MeansUnsprayed$Yield)) {
    ...
    if(n>1) {
      ...
      TTestExample2$Delta[i] <- mean(Differences)
      TTestExample2$SD[i] <- sd(Differences)
      TTestExample2$t[i] <- TTestExample2$Delta[i]/(TTestExample2$SD[i]*sqrt(1/n))
      TTestExample2$P[i] <- 2*(1-pt(abs(TTestExample2$t[i]),n-1))
    } else {
...
TTestExample2$Significant[is.na(TTestExample2$Significant)] <- FALSE
@
...
%  Signficant = rep(TTestExample2$P<0.05,4)
%)
...
geom_point(aes(colour = Significant),size=1) + 
scale_colour_manual(values=cbPalette[pair.colors]) + 
labs(colour = "Significant", x="Easting", y="Yield (bu/acre)", title = "Example 2 Yield Difference")
@
\caption{Pointwise yield differences $\bar{d})(x)$ for Example 2}\label{Example2Differences}
\end{figure}
...
geom_point(aes(colour = Significant),size=1) + 
scale_colour_manual(values=cbPalette[pair.colors]) + 
labs(colour = "Significant", x="Easting", y="Number of Strips", title = "Example 2 n")
@
\caption{Pointwise number of strips $n(x)$ for Example 2}\label{Example2N}
\end{figure}
...
@
\caption{Pointwise yield differences standard deviations $S_d(x)$ for Example 2}\label{Example2SD}
\end{figure}
...
@
\caption{Pointwise $t(x)$ ratio values for Example 2}\label{Example2T}
\end{figure}
...
\cite{piepho-11-2011} proposes a method of analysis of strip trials, according to the following algorithm:
\begin{enumerate}
  ...
  \item spatial analysis of all geo-referenced subsamples based on categorised deflect angles with and without taking covariates into account.
\end{enumerate}
...
\cite{evans-11-2020} used geographically weighted regression for the analysis of strip trials. This method has the advantage of partitioning variance in strip trials into spatial variability and treatment components. \cite{stefanova-04-2023} used a clustering method to delineate strips into comparable pseudo-environments, then analyzed the resulting clusters as multienvironment trials using linear mixed models. As with ISOFAST, these methods sacrifice local spatial analysis for the good of a broader geographical spatial analysis.
...
\pagebreak
\chapter{Pointwise Trend Analysis of Functional Strip Data}\label{ChapterTrend}


<<RelevelExample1Product, echo=FALSE,include=FALSE>>=
#relevel product to make the treatment coefficent positive
...
This can lead to variety effects being confounded with field effects. We can account for this by using a regression analysis to smooth the spatial trend, then add to the trend a term for variety. That is, our model for strip yields takes the form \cite{hastie-1996, wood-2017}

\begin{equation}
y_i = a_i\theta(x) + f(x) + \epsilon_i(x)
\end{equation}\label{UnivariateGAM}

In this case, $a_i$ is an indicator variable for variety. Variety E is the baseline or intercept, $a_i=0$ when the variety planted in strip $i$ is labelled `E`, while $a_i=1$ incorporates the additive effect if the variety planted in strip $i$ is variety labelled `B`. $f(x)$ is a smoothing function produced by the default \texttt{s} specification in \texttt{mgcv} \texttt{gam} function. 

A first pass at using \texttt{gam} to provide a spatial trend smoother, we use strip means for harvest strips. There are two harvest strips for each variety, so there are two means per variety. The split-planter layout was duplicated six times, so there are 24 total mean estimates, two harvest strips per variety and two varieties per planted strip. Additionally, for each strip, the average `Northing` value is taken as the position of the strip. These data were fit to a generalized additive model using syntax of the form \texttt{Yield \textasciitilde Product + s(Northing)}, where \texttt{Product} is the parametric portion of the model and \texttt{Northing} is a spatial covariate. 
...
@
\caption{Point estimates of strip yield with predict yield from a GAM mode}\label{PointEstimatesWithPrediction}
\end{figure}
...
Trends$Signficant = Trends$p.pv<0.05
@

\begin{figure}\centering
<<Example1PointwiseBCoefficients,fig=TRUE,echo=FALSE, width=8, height=3>>=
ggplot(Trends, aes(Easting,p.coeff)) + 
geom_point(aes(color=Signficant)) + 
scale_colour_manual(values=cbPalette) +
labs(x="Easting", y="Yield (bu/acre)", title = "Product B Coefficient")
@
\caption{Pointwise product B coefficients from \texttt{gam} trend analysis.}\label{Example1PointwiseBCoefficients}
\end{figure}
...
@
\caption{Pointwise $t$-test for the coefficients in Figure \ref{Example1PointwiseBCoefficients}}\label{Example1PointwiseBtTests}
\end{figure}
...
@
\caption{Pointwise $p$ values for the $t$ ratios in Figure \ref{Example1PointwiseBtTests}}\label{Example1PointwiseBpValues}
\end{figure}
...
labs(colour = "Variety", x="Easting", y="Yield", title = "East Quarter, Pooled Strips") +
  ylim(100, 240)
@
\caption{Interpolated harvest passes in the region 210-240m \texttt{Easting}}\label{Example1HarvestPasses210}
\end{figure}
...
@
\caption{Pointwise effective degrees of freedom for \texttt{gam} trend analysis in the region of 210-240m \texttt{Easting}}\label{Example1PointwiseEDF210}
\end{figure}
...
labs(x="Easting", y="Effective d.f.", title = "Pointwise Degrees of Freedom")
@
\caption{Pointwise effective degrees of freedom for \texttt{gam} trend analysis}\label{Example1PointwiseEDF}
\end{figure}
...
@
\caption{Effective degrees of freedom associated with pointwise GAM fit to different bases}\label{Example1EDFCompEDF}
\end{figure}


So, to further our investigation of the behavior of \texttt{mgcv}, we consider the results for trend analysis at \texttt{Easting} distances of 214m and 216m.
...
In Figure \ref{SubsetAndAnalyze21416RawDataPlot} we focus on trend analysis at 214 and 216 meters `Northing`. The points are interpolated yield estimates using the \texttt{mgcv} \texttt{gam} function on the 24 yield monitor data curves, while the lines are \texttt{mgcv} \texttt{gam} smoothing curves of `Yield` by `Northing`. We particularly note that the points are nearly superimposed over each other at similar `Northing` positions. This is to be expected since the point estimates are derived from smoothed functional forms of the original data. However, for `Northing` 214, the \texttt{gam} solver finds a curve with 7.7 effective degrees of freedom, while for `Northing` 216, with the same function arguments and nearly the same data, \texttt{gam} finds an interpolating curve with 1 degree of freedom - essentially a straight line for the smoothing part of the model. The curves themselves are not simple lines, since the \texttt{gam} model includes `Variety` in the fixed effects component of the model.
...
@
\caption{Yield estimates by Northing for \texttt{Easting} points 214m and 216m}\label{SubsetAndAnalyze21416RawDataPlot}
\end{figure}

\pagebreak
\section{Adaptive Splines}
...
@
\caption{Interpolated yield values for harvest strips from Example 1 using adaptive splines}\label{Example1SmoothedPassesAD}
\end{figure}
...
labs(x="Easting", y="Yield (bu/acre)", title = "Product B Coefficient")
@
\caption{Pointwise product B coefficients from \texttt{gam} trend analysis using harvest passes interpolated with adaptive splines.}\label{Example1PointwiseBCoefficientsAD}
\end{figure}
...
labs(x="Easting", y="Effective d.f.", title = "Pointwise Degrees of Freedom")
@
\caption{Pointwise effective degrees of freedom for \texttt{gam} trend analysis}\label{Example1PointwiseEDFAD}
\end{figure}
...
\subsection{Pointwise trend analysis using ordinary least squares}
...
for(i in 1:length(smooth.basis)) {
  ...
  Trends$lm.coef3[i] <- coef(current.lm)[2]
  current.lm <- lm(Yield ~ Product + bs(Northing, df=8), data = obs)
  Trends$lm.coef8[i] <- coef(current.lm)[2]
}
...
@
\caption{Coefficients associated with variety B when fit using either \texttt{mgcv} or b-splines with \texttt{lm}}\label{Example1EDFCompCoeffGAMLM}
\end{figure}
...
@
\caption{Pointwise $F$ values computed using the \texttt{gam} library for trend analysis.}\label{Example1TrendAnalysisGAMF}
\end{figure}
...
@
\caption{Pointwise $p$ values associated with the $F$ values in Figure \ref{Example1TrendAnalysisGAMF}}\label{Example1TrendAnalysisGAMp}
\end{figure}
...
@
\caption{Pointwise non-parametric (i.e. \texttt{s}) degrees of freedom for the \texttt{gam} models generating $F$ values in Figure \ref{Example1TrendAnalysisGAMF}}\label{Example1TrendAnalysisGAMdf}
\end{figure}
...
\pagebreak
\section{The \texttt{brms} package}
...
if(!file.exists("Trend.brm")) {
  ...
  for(i in 1:length(smooth.basis)) {
    l <- smooth.basis[i]
    obs <- Example1SmoothHarvestPasses.dat[Example1SmoothHarvestPasses.datt$Easting==l,]
...
@
\caption{Pointwise product B coefficient and confidence intervals estimated using the \texttt{brm} library}\label{CITrendCoefficientBRM}
\end{figure}
...
\subsection {Empirical Semivariogram Estimators}
...
There are a number of parametric models for semivariograms, including those known as the exponential model, the Gaussian model and the spherical model. We won't be estimate parameters of the semivariogram from data in this study (such estimates are used for other types of spatial analysis such as kriging), but we will illustrate an idealized semivariogram based on the spherical model (Figure \ref{IdealSemivariogram}. As representative of most parametric functions for the semivariogram, the spherical model takes the form
...
\begin{figure}\centering
<<IdealSemivariogram,fig=TRUE,echo=FALSE, width=8, height=6>>=
...
We can use the yield monitor data from example directly, specifying a response surface GAM model with variety effects as
...
@
\caption{Spraying and yield monitor harvest maps for Example 2}\label{Example2FullMaps}
\end{figure}
...
\subsection{Fitting response surface models for Example 1}
...
#predict a spatial response surfice as if only "E" were planted. This reduces noise between rows of different varieties
...
if(!file.exists("ComputeExample1LMSpatial.Rda")) {
  ...
  for(j in 1:foldgroups) {
    ...
    if(j*10>maxidx) {
      range = ((j-1)*10):maxidx
      Example1ColumnSubset$Fold[range] <- sample(1:length(range))
    } else {
      Example1ColumnSubset$Fold[range] <- sample(1:10)
    }
  }
...
@
\caption{Predicted yield response surfaces from linear model with b-splines for Example 1}\label{Example1LMPredictions}
\end{figure}
...
labs(colour = "Yield", x="Easting", y="Northing") + facet_wrap(~ K)
@
\caption{Predicted yield response surfaces from \texttt{mgcv} model with default bases for Example 1}\label{Example1GAMPredictions}
\end{figure}
...
Example1VariogramSelectionGAM$Nugget[1] <- coef(tmp.lm)[1]
sum <- summary(tmp.lm)
Example1VariogramSelectionGAM$NuggetT[1] <- sum$coefficients[1,'t value']
Example1VariogramSelectionGAM$NuggetP[1] <- sum$coefficients[1,'Pr(>|t|)']
  ...
  for (k in 1:length(Example1VariogramSelectionGAM$K)) {
    ...
    Example1VariogramSelectionGAM$Intercept[k] <- coef(tmp.lm)[1]
    Example1VariogramSelectionGAM$Slope[k] <- coef(tmp.lm)[2]
    sum <- summary(tmp.lm)
    Example1VariogramSelectionGAM$SlopeT[k] <- sum$coefficients['Distance','t value']
    Example1VariogramSelectionGAM$SlopeP[k] <- sum$coefficients['Distance','Pr(>|t|)']
    tmp.lm <- lm(Gamma ~ bs(Distance),data=Example1PredictionGAMVariogramPars[Example1PredictionGAMVariogramPars$K==K,])
    Example1VariogramSelectionGAM$Nugget[k] <- coef(tmp.lm)[1]
    sum <- summary(tmp.lm)
    Example1VariogramSelectionGAM$NuggetT[k] <- sum$coefficients[1,'t value']
    Example1VariogramSelectionGAM$NuggetP[k] <- sum$coefficients[1,'Pr(>|t|)']
  }
...
sum <- summary(tmp.lm)
Example1VariogramSelectionLM$NuggetT[1] <- sum$coefficients[1,'t value']
Example1VariogramSelectionLM$NuggetP[1] <- sum$coefficients[1,'Pr(>|t|)']
  ...
  for (k in 1:length(Example1VariogramSelectionLM$K)) {
    ...
    Example1VariogramSelectionLM$Intercept[k] <- coef(tmp.lm)[1]
    Example1VariogramSelectionLM$Slope[k] <- coef(tmp.lm)[2]
    sum <- summary(tmp.lm)
    Example1VariogramSelectionLM$SlopeT[k] <- sum$coefficients['Distance','t value']
    Example1VariogramSelectionLM$SlopeP[k] <- sum$coefficients['Distance','Pr(>|t|)']
    tmp.lm <- lm(Gamma ~ bs(Distance),data=Example1PredictionLMVariogramPars[Example1PredictionLMVariogramPars$K==K,])
    Example1VariogramSelectionLM$Nugget[k] <- coef(tmp.lm)[1]
    sum <- summary(tmp.lm)
    Example1VariogramSelectionLM$NuggetT[k] <- sum$coefficients[1,'t value']
    Example1VariogramSelectionLM$NuggetP[k] <- sum$coefficients[1,'Pr(>|t|)']
  }
...
\subsection{Example 1 Model Selection}
...
Example1PredictionGAMVariogramPars$Source <- "Reponse Surface"
...
@
\caption{Variograms for GAM basis spline models fit to data from Example 1}\label{Example1VariogramsGAMPlot}
\end{figure}
...
@
\caption{Selection criteria for linear regression fit basis splines models}\label{Example1ModelSelectionLM}
\end{figure}
...
@
\caption{Selection criteria for GAM fit basis splines models}\label{Example1ModelSelectionGAM}
\end{figure}
...
\subsection{Example 1 Variety Coefficients}
...
for(i in 1:length(SpatialGamKs)) {
  coefs <- coef(Example1GAMModels[[i]])
  Coef <- c(Coef,coefs[2])
  Source <- c(Source, "GAM")
...
grid.arrange(
  arrangeGrob(
     ggplot(Plot1[Plot1$Source == "GAM",], aes(K,Coef)) + 
         geom_point(size=2) + 
         geom_line(linewidth=1) + 
         scale_colour_manual(values=cbPalette) + 
         labs(title="GAM Coefficients, Example 1", x ="Smoothing Parameter", y = "Product Effect Estimate"),
...
@
\caption{Coefficients from \texttt{mcgv} and \texttt{lm} fitting b-splines for spatial models fit to Example 1}\label{Example1Coefficents}
\end{figure}
...
\subsection{Fitting response surface models for Example 2}
...
labs(colour = "Yield", x="Easting", y="Northing") + facet_wrap(~ K)
@
\caption{Predicted yield response surfaces from \texttt{mgcv} model with default bases for Example 2}\label{Example2FullGAMPredictions}
\end{figure}

Response surface estimates for the full data set for Example 2 is shown in Figure \ref{Example2FullGAMPredictions}. As with Figure \ref{Example1GAMPredictions}, we see increasing detail in the predicted response surface yield structure. By inspection, \texttt{k=180} appears optimal.
...
Example2FullVariogramSelectionGAM$Nugget[1] <- coef(tmp.lm)[1]
sum <- summary(tmp.lm)
Example2FullVariogramSelectionGAM$NuggetT[1] <- sum$coefficients[1,'t value']
Example2FullVariogramSelectionGAM$NuggetP[1] <- sum$coefficients[1,'Pr(>|t|)']
  ...
  for (k in 1:length(Example2FullVariogramSelectionGAM$K)) {
    ...
    Example2FullVariogramSelectionGAM$Intercept[k] <- coef(tmp.lm)[1]
    Example2FullVariogramSelectionGAM$Slope[k] <- coef(tmp.lm)[2]
    sum <- summary(tmp.lm)
    Example2FullVariogramSelectionGAM$SlopeT[k] <- sum$coefficients['Distance','t value']
    Example2FullVariogramSelectionGAM$SlopeP[k] <- sum$coefficients['Distance','Pr(>|t|)']
    tmp.lm <- lm(Gamma ~ bs(Distance),data=Example2FullPredictionGAMVariogramPars[Example2FullPredictionGAMVariogramPars$K==K,])
    Example2FullVariogramSelectionGAM$Nugget[k] <- coef(tmp.lm)[1]
    sum <- summary(tmp.lm)
    Example2FullVariogramSelectionGAM$NuggetT[k] <- sum$coefficients[1,'t value']
    Example2FullVariogramSelectionGAM$NuggetP[k] <- sum$coefficients[1,'Pr(>|t|)']
  }
...
%    Example2FullVariogramSelectionLM$Intercept[k] <- coef(tmp.lm)[1]
%    Example2FullVariogramSelectionLM$Slope[k] <- coef(tmp.lm)[2]
%    sum <- summary(tmp.lm)
%    Example2FullVariogramSelectionLM$SlopeT[k] <- sum$coefficients['Distance','t value']
%    Example2FullVariogramSelectionLM$SlopeP[k] <- sum$coefficients['Distance','Pr(>|t|)']
%    tmp.lm <- lm(Gamma ~ bs(Distance),data=Example2FullPredictionLMVariogramPars[Example2FullPredictionLMVariogramPars$K==K,])
%    Example2FullVariogramSelectionLM$Nugget[k] <- coef(tmp.lm)[1]
%    sum <- summary(tmp.lm)
%    Example2FullVariogramSelectionLM$NuggetT[k] <- sum$coefficients[1,'t value']
%    Example2FullVariogramSelectionLM$NuggetP[k] <- sum$coefficients[1,'Pr(>|t|)']
%  }
...
ggplot(side.dat, aes(Distance,Gamma)) + 
    ...
    facet_wrap(~ Source, scales = "free")
@
\caption{Variograms for GAM basis spline models fit to data from Example 2}\label{Example2FullVariogramsGAMPlot}
\end{figure}
...
\subsection{Example 2 Model Selection}
...
ggplot(Example2FullModelSelectionGAM, aes(K,Score)) + 
    ...
    labs(title="Scores for GAM smoothers", x ="Smoothing Parameter", y = "Score")
@
\caption{Selection criteria for \texttt{mgcv} fit response surface models}\label{Example2FullModelSelectionGAM}
\end{figure}
...
\subsection{Example 2 Fungicide Coefficients}
...
for(i in 1:length(SpatialGamKs)) {
  coefs <- coef(Example2FullGAMModels[[i]])
  Coef <- c(Coef,coefs[2])
  K <- c(K,SpatialGamKs[i])
}
Plot2 <- data.frame(Coef=Coef,K=K)
@
...
@
\caption{Coefficients from \texttt{mcgv} response surface models fit to Example 2}\label{Example2FullCoefficents}
\end{figure}
...
labs(colour = "Yield", x="Easting", y="Northing") + facet_wrap(~ K)
@
\caption{Predicted yield response surfaces from \texttt{brms} model with default bases for Example 2}\label{Example1BRMPredictions}
\end{figure}
...
Example1VariogramSelectionBRM$Nugget[1] <- coef(tmp.lm)[1]
sum <- summary(tmp.lm)
Example1VariogramSelectionBRM$NuggetT[1] <- sum$coefficients[1,'t value']
Example1VariogramSelectionBRM$NuggetP[1] <- sum$coefficients[1,'Pr(>|t|)']
  ...
  for (k in 1:length(Example1VariogramSelectionBRM$K)) {
    ...
    Example1VariogramSelectionBRM$Intercept[k] <- coef(tmp.lm)[1]
    Example1VariogramSelectionBRM$Slope[k] <- coef(tmp.lm)[2]
    sum <- summary(tmp.lm)
    Example1VariogramSelectionBRM$SlopeT[k] <- sum$coefficients['Distance','t value']
    Example1VariogramSelectionBRM$SlopeP[k] <- sum$coefficients['Distance','Pr(>|t|)']
    tmp.lm <- lm(Gamma ~ bs(Distance),data=Example1PredictionBRMVariogramPars[Example1PredictionBRMVariogramPars$K==K,])
    Example1VariogramSelectionBRM$Nugget[k] <- coef(tmp.lm)[1]
    sum <- summary(tmp.lm)
    Example1VariogramSelectionBRM$NuggetT[k] <- sum$coefficients[1,'t value']
    Example1VariogramSelectionBRM$NuggetP[k] <- sum$coefficients[1,'Pr(>|t|)']
  }
  
  Example1ModelSelectionBRM <- data.frame(K = rep(c(0,SpatialGamKs),4),
                           #Criteria = as.factor(c(rep('AIC',length(SpatialGamKs)+1),
                           #                        rep('BIC',length(SpatialGamKs)+1),
                            Criteria = as.factor(c(rep('Adjusted R Squared',length(SpatialGamKs)+1),
                                                   rep('Generalized Cross Validation',length(SpatialGamKs)+1),
...
@
\caption{Selection criteria for \texttt{brm} fit response surface models}\label{Example1ModelSelectionBRM}
\end{figure}
...
@
\caption{Coefficients from \texttt{brms} response surface models fit to Example 1}\label{Example1BRMCoefficents}
\end{figure}
...
<<SpatialFusionPlaceholder,eval=FALSE,include=FALSE,echo=FALSE>>=
...
@
\caption{Spraying and yield monitor harvest maps for a subset of Example 2}\label{Example2SubMaps}
\end{figure}
...
labs(colour = "Yield", x="Easting", y="Northing") + facet_wrap(~ K)
@
\caption{Predicted yield response surfaces from \texttt{mgcv} model with default bases fora subset of Example 2}\label{Example2SubGAMPredictions}
\end{figure}
...
Example2SubVariogramSelectionGAM$Nugget[1] <- coef(tmp.lm)[1]
sum <- summary(tmp.lm)
Example2SubVariogramSelectionGAM$NuggetT[1] <- sum$coefficients[1,'t value']
Example2SubVariogramSelectionGAM$NuggetP[1] <- sum$coefficients[1,'Pr(>|t|)']
  ...
  for (k in 1:length(Example2SubVariogramSelectionGAM$K)) {
    ...
    Example2SubVariogramSelectionGAM$Intercept[k] <- coef(tmp.lm)[1]
    Example2SubVariogramSelectionGAM$Slope[k] <- coef(tmp.lm)[2]
    sum <- summary(tmp.lm)
    Example2SubVariogramSelectionGAM$SlopeT[k] <- sum$coefficients['Distance','t value']
    Example2SubVariogramSelectionGAM$SlopeP[k] <- sum$coefficients['Distance','Pr(>|t|)']
    tmp.lm <- lm(Gamma ~ bs(Distance),data=Example2SubPredictionGAMVariogramPars[Example2SubPredictionGAMVariogramPars$K==K,])
    Example2SubVariogramSelectionGAM$Nugget[k] <- coef(tmp.lm)[1]
    sum <- summary(tmp.lm)
    Example2SubVariogramSelectionGAM$NuggetT[k] <- sum$coefficients[1,'t value']
    Example2SubVariogramSelectionGAM$NuggetP[k] <- sum$coefficients[1,'Pr(>|t|)']
  }
...
%    Example2SubVariogramSelectionLM$Intercept[k] <- coef(tmp.lm)[1]
%    Example2SubVariogramSelectionLM$Slope[k] <- coef(tmp.lm)[2]
%    sum <- summary(tmp.lm)
%    Example2SubVariogramSelectionLM$SlopeT[k] <- sum$coefficients['Distance','t value']
%    Example2SubVariogramSelectionLM$SlopeP[k] <- sum$coefficients['Distance','Pr(>|t|)']
%    tmp.lm <- lm(Gamma ~ bs(Distance),data=Example2SubPredictionLMVariogramPars[Example2SubPredictionLMVariogramPars$K==K,])
%    Example2SubVariogramSelectionLM$Nugget[k] <- coef(tmp.lm)[1]
%    sum <- summary(tmp.lm)
%    Example2SubVariogramSelectionLM$NuggetT[k] <- sum$coefficients[1,'t value']
%    Example2SubVariogramSelectionLM$NuggetP[k] <- sum$coefficients[1,'Pr(>|t|)']
%  }
...
@
\caption{Selection criteria for GAM fit basis splines models}\label{Example2SubModelSelectionGAM}
\end{figure}
...
ggplot(side.dat, aes(Distance,Gamma)) + 
    ...
    facet_wrap(~ Source, scales = "free")
@
\caption{Variograms for GAM basis spline models fit to data from Example 1}\label{Example2SubVariogramsGAMPlot}
\end{figure}
...
for(i in 1:length(SpatialGamKs)) {
  coefs <- coef(Example2SubGAMModels[[i]])
  Coef <- c(Coef,coefs[2])
  K <- c(K,SpatialGamKs[i])
}
Plot2 <- data.frame(Coef=Coef,K=K)
...
\begin{figure}\centering
<<Example2SubCoefficents,fig=TRUE,echo=FALSE, width=8, height=6>>=
ggplot(Plot2, aes(K,Coef)) + 
      geom_point(size=2) + 
      geom_line(linewidth=1) + 
      scale_colour_manual(values=cbPalette) + 
...
@
\caption{Coefficients from \texttt{mcgv} and response surface models fit to a subset of Example 2}\label{Example2SubCoefficents}
\end{figure}
...
That is, for Example 1, the answer to the question "Did the new product work well enough in the strip trial for the collaborator to adopt the new technology for application to a larger percent of his total acreage?" is quite obvious, even in the abscence of statistical analysis. For Example 2, the answer is more ambiguous. For Example 2, at best fungicide spray had no meaningful effect, and in the worst case may have cost the farmer oof nearly 4 bu/acre.
