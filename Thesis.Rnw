...
\begin{abstract}
Collaborative on-farm trials of agronomic practices have been an established method of interaction between researchers and their target audience (the farmers). With the advent of precision agriculture practices, the on-farm strip trial has become a popular method of collaborative research. Precision agriculture practices allows farmers to precisely control the application of an agronomic product or practice to user specified strips in a cropland, and to leave as standard practice strips, or check strips, intermingled with the test strips.

In spite of their popularity, there has been little development in the methods of analyzing strip trials. In this thesis, we consider two examples of strip trials. The first is considered an ideal case of a strip trial, where the test strips are long and narrow and together with the check strips form a uniform rectangle. With this example, we develop methods of analyzing strips as functional data objects and apply functional paired $t$-tests and functional trend analysis. 

The second example is a strip trial where the test product was applied more haphazardly and performs poorly when the strips are analyzed as functional data. To provide an analysis of these data, we extend the trend analysis to a two-dimensional spatial analysis as a method to determine the effect of the applied product.
...
<<StartTiming, eval=FALSE, include=FALSE, echo=FALSE>>=
#StartTypesetting <- Sys.time()
#Rprof()
@
...
In Chapter \ref{ChapOnFarmStripTrials} we first review the literature of on-farm strip trial analysis and discuss the nature of yield monitor data, followed by Chapter \ref{RepresentativeStripTrialsChapter} describing two representative strip trials that will serve as sources for example data analyzed in this thesis. We introduce data before completing the relevant literature review so that we can use examples of functional data from representative trials to motivate later chapters.

Chapter \ref{UnivariateAndFunctionalDataAnalysisChapter} describes the similarity to traditional \textit{univariate} data analysis to pointwise functional data analysis. We then consider in Chapter \ref{SmoothingFunctionsChapter}  how temporal data collected during harvest (harvest passes) can be modeled using spline bases functions. 
...
We will use the term crop-land for an area of ground planted and harvested by a farmer. The term \textit{field} is more commonly used, but we will also be discussing mathematical entities known as \textit{random fields}, so we will be using \textit{field} to imply the latter context, and \textit{crop-land} for the former.
...
Each strip may be thought of in terms of a traditional plot, albeit a very long and very thin plot, and in some cases the results of a strip trial is presented in a manner analogous to traditional small-plot trials \cite{kandel-2018, laurent-2022}. This practice has a long history\cite{wishart-1934}. However, a strip in the shape a very long and very thin rectangle becomes increasingly similar to a single line, and we can easily model yield as function of position on this line. Thus, the analysis of yield monitor data from strip trials is a likely candidate 
...
Evans et al. \cite{evans-11-2020} used geographically weighted regression for the analysis of strip trials. This method has the advantage of partitioning variance in strip trials into spatial variability and treatment components. Stefanova, et al. \cite{stefanova-04-2023} used a clustering method to delineate strips into comparable pseudo-environments, then analyzed the resulting clusters as multi-environment trials using linear mixed models. As with ISOFAST, these methods sacrifice local spatial analysis for the good of a broader geographical spatial analysis.
...
The examples used to present a method of functional analysis are better understood as systematic strip arrangements, as opposed to more traditionally randomized trials. Randomization is key if causal effects are to be investigated, but randomization may not always be practical, so in many cases farmers are recommended to use systematic strips. The relative merits of systematic arrangements versus randomized arrangements has been debated for much longer than strip trials have been made avaiable to farmers \cite{yates-01-1939}. If trends are accounted for, systematic trials can be effective \cite{alesso-11-2019, cao-06-2022}. In the chapter \ref{TrendAnalysisChapter}, we consider trend analysis incorporated into functional data analysis of systematic trials.
...
The second example was executed in a cropland that was not a simple rectangle, such that experimental units are not of the same length. This example represents a case with extensive naturally occuring yield variability \cite{grisso-04-2009} that complicate interpretation of yield maps. This cropland also had areas that were not planted, making this experiment something of a worse-case scenario for obtaining meaningful results that can be communicated to the farmer-collaborator. 
...
For both examples, limited data cleaning was performed, primarily in the form of screening for outliers. More extensive methods of cleaning data have been proposed, taking advantage of knowledge of the types of measurement errors associated with combined yield monitors \cite{wiebold-07-2003, simbahan-2004, ping-2005, luck-06-2015, kharel-03-2019}, however, the most popular software \cite{sudduth-11-2007, sudduth-07-2012} for automated screening of yield monitor data is not available for MacOS (which was used for these analysis) and is dated; version 2.0 was not available at the time of this writing.
...
@
\caption{Representative harvest pass with no smoothing. These data are the 24$^{th}$ or northern most of data shown in Figure \ref{RawSwaths}. Instantaneous yield estimates are shown as gray points. Yield estimates are taken a 1 second intervals, but we are interested in the location of estimates, so points are plotted versus meters \texttt{Easting}}\label{RawSwath}
\end{figure}
...
Several mathematical models exist that are used to approximate $f$. In this section we review some possible functional forms, again following \cite{wood-2017}, p. 162. We define a *basis* as a space of functions which includes $f$ (or, at least, a close approximation) as an element. A basis is defined by some set of functions $b_j(x)$, $j = 1, \cdots, k$. We then represent $f$ as a sum of the basis functions,
\begin{equation}
...
PredictionsLM.dat[,'Order'] <- factor(rep(knots, each = length(SinglePass.dat[,'Easting'])))
PredictionsLM.dat[,'Yield'] <- NA

for(i in 2:length(knots[1:5])) {
  current.lm <- lm(Yield ~ poly(Easting, knots[i]), data=SinglePass.dat)
  PredictionsLM.dat[PredictionsLM.dat[,'Order']==knots[i], 'Yield'] <- predict(current.lm)
}
...
#  geom_line(aes(Easting, Yield),size = 0.5, data=SinglePass.dat, color=cbPalette[1]) + 
  geom_line(linewidth=1, aes(color=Order, linetype=Order)) + 
  scale_colour_manual(values=cbPalette[2:11]) + 
  labs(colour = 'Order', linetype = 'Order', x='Easting', y='Yield', title = 'Single harvest pass, polynomial smoothing')
@
...
An alternative approach, piecewise linear regression, partitions the data into \textit{bins}, then fit a linear model restricted to the bin \cite{fox-1997}. The points at which the bins join are referred to as knots \cite{wood-2017}, while the basis for this interpolation is a series of simple linear function.
...
\begin{equation}
   B_j^m(x) = \frac{x-x_j}{x_j+m+1}B^{m-1}(x) + \frac{x_{j+m+2}-x}{x_{i+m+2}-x_j+1}B^{m-1}{j+1}(x), i = 1, \cdots, k
\end{equation}
with
\begin{equation}
   B^{-1}_j(x) = 
...
\texttt{bs} chooses \texttt{df-degree} knots if \texttt{df} is passed an a argument. We illustrate the \texttt{bs} function by using the argument \texttt{df=15}. B-splines with 12+3=15 \texttt{df} identifies similar features as with \texttt{ns} with 12+1 = 13 degrees of freedom.

<<SplineSmoothingCode, echo=FALSE>>=
spline.knots = seq(0,1, length.out=12)
ns12_1.lm <- lm(Yield ~ ns(Easting, df=12+1), data=SinglePass.dat,x=TRUE)
ns12_1.X <- ns12_1.lm$x
...
bs12_3.lm <- lm(Yield ~ bs(Easting, df=12+3), data=SinglePass.dat,x=TRUE)
bs12_3.X <- bs12_3.lm$x
#dim(bs12_3.X)
...
\begin{figure}\centering
<<NSBasisPlot, fig=TRUE, echo=FALSE, width=8, height=4>>=
#first column is the intercept, so exclude that
ns.funcs <- dim(ns12_1.X)[1]
points <- length(SinglePass.dat$Easting)
NSBasis.dat <- data.frame(X=rep(SinglePass.dat$Easting,14),
                          Basis=c(rep(1, points),
                                  rep(2, points),
                                  rep(3, points),
                                  rep(4, points),
                                  rep(5, points),
...
BSBasis.dat <- data.frame(X=rep(SinglePass.dat$Easting,16),
                          Basis=c(rep(1, points),
                            ...
                            Spline = c(bs12_3.X[,1],
                                       ...
                                       bs12_3.X[,15],
                                       bs12_3.X[,16]))
BSBasis.dat$Basis <- as.factor(BSBasis.dat$Basis)
ggplot(aes(X,Spline),data=BSBasis.dat) + 
  geom_line(aes(col=Basis)) + 
  scale_colour_manual(values=cbPalette) +
  guides(color = guide_legend(ncol = 2)) 
@ 
\caption{Basis values from the $X$ design matrix for the spline \texttt{bs(12+3)} used to illustrate the basis spline fit in Figure \ref{ComparingSplineFits}. See Figure \ref{NSBasisPlot} for a description of the lines. Columns 4-13 define simple bell-shaped curves similar to those shown in Figure \ref{NSBasisPlot}, while columns 2, 3, 14 and 15 define skewed bell curves at endpoints of the range of $x_j$ and column 1 is a constant or intercept} \label{BSBasisPlot}. Note that for \texttt{12+1} d.f. for the natural spline we find 14 basis functions, while for b-splines of 12+3 d.f. we find 16 basis functions (see Figure \ref{BSBasisPlot}), thus we can consider d.f. to be one less than the number of basis functions. Remember that in Figure \ref{ComparingSplineFits} the arguments of \texttt{df=12+1} to \texttt{ns} and \texttt{df=12+3} to \texttt{bs} provided very similar curves.
\end{figure}

We can illustrate the concept of basis functions easily with design matrices \texttt{X} extracted from \texttt{lm} fit objects; we simply obtain \texttt{X} from an object returned by \texttt{lm}. It is not so convenient to show the basis functions fit using \texttt{mgcv}, so only the basis functions for the \texttt{splines} fit using \texttt{lm} are illustrated in Figures \ref{NSBasisPlot} and \ref{BSBasisPlot}. Let $\mathbf{y} = \mathbf{X} \mathbf{\beta}$. Figure \ref{NSBasisPlot} for \texttt{X} matrix expanded from the \texttt{ns} function, while Figure \ref{BSBasisPlot} shows the \texttt{X} matrix expanded from the \texttt{bs} function. \cite{ramsay-2005} makes the point that B-splines are cubic splines with the support of each spline on the interval $[\tau_{k-2}, \tau_{k=2}]$; that is, the sample points where the basis function is non-zero extends beyond the boundary knots for the basis function. While they do not make an explicit statement, we see from Figure \ref{NSBasisPlot} the same can be said for natural splines.

\begin{figure}\centering
<<BSBasisPlot2, fig=TRUE, echo=FALSE, width=8, height=4>>=
bs12_3coeffs <- coef(bs12_3.lm)
BSBasis2.dat <- data.frame(X=rep(SinglePass.dat$Easting,16),
                          Basis=c(rep(1, points),
                                  rep(2, points),
                                  rep(3, points),
                                  rep(4, points),
                                  ...
                                  rep(15, points),
                                  rep(16, points)),
                            Spline = c(bs12_3coeffs[1]*bs12_3.X[,1],
                                       bs12_3coeffs[2]*bs12_3.X[,2],
                                       bs12_3coeffs[3]*bs12_3.X[,3],
                                       bs12_3coeffs[4]*bs12_3.X[,4],
                                       bs12_3coeffs[5]*bs12_3.X[,5],
                                       bs12_3coeffs[6]*bs12_3.X[,6],
                                       bs12_3coeffs[7]*bs12_3.X[,7],
                                       bs12_3coeffs[8]*bs12_3.X[,8],
                                       bs12_3coeffs[9]*bs12_3.X[,9],
                                       bs12_3coeffs[10]*bs12_3.X[,10],
                                       bs12_3coeffs[11]*bs12_3.X[,11],
                                       bs12_3coeffs[12]*bs12_3.X[,12],
                                       bs12_3coeffs[13]*bs12_3.X[,13], 
                                       bs12_3coeffs[14]*bs12_3.X[,14],
                                       bs12_3coeffs[15]*bs12_3.X[,15],
                                       bs12_3coeffs[16]*bs12_3.X[,16]))
BSBasis2.dat$Basis <- as.factor(BSBasis.dat$Basis)
ggplot(aes(X,Spline),data=BSBasis2.dat) + 
  geom_line(aes(col=Basis)) + 
  scale_colour_manual(values=cbPalette) +
  guides(color = guide_legend(ncol = 2))
@
...
We consider the \texttt{bs} basis functions and resulting functional form in more detail. In mathematical notation, let $x_1, x_2, \dots x_n$ be the sample points for the data in Figure \ref{RawSwath} and $X_{1,1}, X_{2,1}, \dots, X_{k,1}$ be the non-zero elements of first column \texttt{X} with $X_{k+1,1} = X_{k+2,1} = \dots X_{n,1} = 0$. More generally, the values in \texttt{X} will be the basis functions evaluated at the sampling ponts, in the case of b-splines ths will be of the form $X_{i,j} = B^m_j(x_i)$. The values $X_{1,1}, X_{2,1}, \dots, X_{k,1}, X_{k+1,1}, X_{k+2,1}, \dots X_{n,1}$ are plotted as the second curve in Figure \ref{BSBasisPlot}, while  are plotted in Figure 

We multiply each basis column by it's associated estimated $\widehat{\mathbf{\beta}}$ coefficients, that is, $\beta_{1} X_{1,1}, \beta_{1} X_{2,1}, \dots, \beta_{1} X_{k,1}, \beta_{1} X_{k+1,1}, \beta_{1} X_{k+2,1}, \dots \beta_{1} X_{n,1}$. $\widehat{\mathbf{\beta}}$ extracted from the associated \texttt{lm} object using the \texttt{coef} function. These curves are shown in Figure \ref{BSBasisPlot2}. From this, we can reconstruct the curve in Figure \ref{ComparingSplineFits} by summing over all $X$ rows for each sampling point $x_i$. This can be achieved by multiply the design matrix $X$ by the coefficients $\widehat{\beta}$ and plotting $\widehat{y_i}$ against $x_i$, as show in Figure \ref{BSBasisPlot3}.

\begin{figure}\centering
<<BSBasisPlot3, fig=TRUE, echo=FALSE, width=8, height=4>>=
BSBasis3.dat <- data.frame(X=SinglePass.dat$Easting,
                           Predicted=(bs12_3.X %*% bs12_3coeffs)[,1]) 
ggplot(aes(X,Predicted),data=BSBasis3.dat) + 
  geom_line() + 
...
Now, reconsider the piecewise linear basis in Equation \ref{PiecewiseLinearBasis}. Instead of minimizing $\lVert\mathbf{y} - \mathbf{X} \mathbf{\beta} \rVert ^2$, we may instead choose to control the wiggliness of the resulting fit model by minimizing a penalized objectiv function \cite{wood-2017}
\begin{equation}
...
\end{equation}\label{SmoothingSplinePenalty}.
...
where $\widehat{f}$ is fit to the entire data set, and $\left\{ A_{ii} \right\} = \mathbf{A}$ is the influence matrix for $\widehat{f}$. $\mathbf{A}$ is computed as
\begin{equation}
\mathbf{A} = \mathbf{X} \left( \mathbf{X}^t \mathbf{X} + \lambda \mathbf{S} \right)^{-1} \mathbf{X}^t   
\end{equation}\label{InfluenceMatrix} with $\mathbf{S} = \mathbf{D}^t \mathbf{D}$ where $\mathbf{D}$ is formed from the columns of $\mathbf{X}$ \cite{wood-2017}.
...
The takeaway from this is that when a model is fit using \texttt{mgcv}, the user does not need to select $\lambda$, instead \texttt{mgcv} finds a solution that minimized the generalized cross validation score. However, in other cases the generalized cross validation score is used to select the number of knots for a penalized spline function \cite{friedman-02-1989, ruppert-01-2002}.
...
Why is there a difference in degrees of freedom for the basis splines model, \texttt{lm} plus \texttt{bs} and  \texttt{mgcv} with \texttt{bs='bs',k=12+4}? From \cite{wood-2017}, we consider the bias corrected fitted values 
\begin{equation}
...
ggplot(ComparisonMGCV.dat, aes(Easting, Yield)) + 
   ...
   scale_colour_manual(values=cbPalette[2:11])
@
\caption{Different spline smoothers using \texttt{gam} with default \texttt{k} values. The default for  \texttt{mgcv} is to use \texttt{thin-plate} splines. Model estimates using \texttt{bs} and \texttt{ns} splines are similar to the default, but not exactly overlapping, Similar \texttt{k} value used as default for the first three bases, but each bases has different constraints on the endpoints. The  \texttt{ad} option uses a larger default \text{k} value and results in a curve that hews more closely to the original data, to the point of overfitting.}\label{ComparingGAMSplines}
\end{figure}
...
\begin{equation}
   \int \dots \int_{\mathcal{R}^d}
   \sum_{\nu_1 + \dots + \nu_d = m} 
   \frac{m!}{\nu_1 ! \dots \nu_d !}
   \left( \frac{\partial ^m g}{\partial x_1^{\nu_1} \dots \partial x_d^{\nu_d}}\right)^2
   dx_1 \dots d x_d
\end{equation}
It is common to see thin-plate splines extended to non-planar surfaces \cite{bookstein-06-1989, keller-02-2019}. 

Only two arguments can supplied to the \texttt{s} function, the covariate \texttt{Easting} and the basis function, that is, the function invocation takes the form \texttt{gam(Yield \textasciitilde s(Easting))} or \texttt{gam(Yield \textasciitilde s(Easting,bs='bs'))}, etc..
...
ggplot(ComparisonBRM.dat, aes(Easting,Yield)) + 
   ...
   scale_colour_manual(values=cbPalette[2:11])
@
\caption{Different spline smoothers using \texttt{brm} default \texttt{k} values. \texttt{brm} uses \texttt{mgcv} to code the bases functions, so we expect to see a great similarity between this graph and Figure \ref{ComparingGAMSplines}. However, we find an error when trying to implement the \texttt{ad} basis in \texttt{brm}, so that curve is not included in this graph.}\label{ComparingBRMSplines}
\end{figure}

The \texttt{brms} library \cite{burkner_2017} provides an interface for specifying multilevel Bayesian model, implemented in the \texttt{stan} programming language \cite{carpenter-02-2015}. While we do not necessarily intend to analyze strip trials in a fully Bayesian manner, the \texttt{brm} function also supports the \texttt{s} smoothing syntax; where the \texttt{s} functionality is derived from \texttt{mgcv}. Figure \ref{ComparingBRMSplines} compares different spline bases when computed used \texttt{brm} as opposed to \texttt{mgcv}. We should note that when we use the \texttt{bs='ad'} syntax in a \texttt{brm} model, we generate an error
\begin{verbatim}
...
We also fit interpolation curves using the \texttt{fda} \cite{ramsay-2024} package. The \texttt{fda} package is useful for various forms of functional data analysis and includes several families of interpolating functions. The \texttt{fda} package requires more function calls than the other packages considered, First, the user must create an explicit set of points to evaluate the function, plus an explicit vector of knots. After these were created, we called the the \texttt{create.bspline.basis} to create a b-spline basis and \texttt{smooth.basis} for compute coefficients. We use for the number of knots in the \texttt{fda} smooth basis values in \texttt{(3,6,12,18,24,30,36)}. We find the the basis functions created by the \texttt{fda} library (Figure \ref{FDABasisPlot}) do not correspond to the basis functions created by the \texttt{splines} \texttt{bs} function (Figure \ref{BSBasisPlot}). This is a troubling inconsistency in the implementation of two commonly used libraries.

\begin{figure}\centering
<<FDABasisPlot, fig=TRUE, echo=FALSE, width=8, height=4>>=
plot.knots <- seq(min(SinglePass.dat[,'Easting']),max(SinglePass.dat[,'Easting']),length.out=12+2)
plot.basis <- create.bspline.basis(c(min(times_basis),max(times_basis)),length(plot.knots) + 4 - 2, 4, plot.knots)
plot(plot.basis)
@
\caption{Basis functions created using the \texttt{fda} library, using code adapted from \cite{kokoszka-2017}. Compare these curves with those presented in Figure \ref{BSBasisPlot}. Specifically, note that the non-bell shaped curve is on the both sides of Figure \ref{FDABasisPlot}, while it is on only the left hand side of Figure \ref{BSBasisPlot}. In contrast to Figure \ref{BSBasisPlot}, this figure does not include a constant curve at the top; there may be a constant function in this graph at the bottom but it is hard to distinquish. Comparing Figures \ref{BSBasisPlot} and \ref{FDABasisPlot} we see some inconsistencies with regard to how splines are implemented in different R libraries.}\label{FDABasisPlot}
\end{figure}
...
PredictionsGAMPS.dat <- PredictionsGAMPS.dat[!is.na(PredictionsGAMPS.dat[,'Yield']),]
...
PredictionsGAMAD.dat <- PredictionsGAMAD.dat[!is.na(PredictionsGAMAD.dat[,'Yield']),]
...
@
\caption{Smoothing curves for a single harvest swath using \texttt{mgcv gam} with the \texttt{bs='ad'} smoother and \texttt{k = Knots+4}. We find that we cannot fit a \texttt{k} argument smaller than \texttt{k=12}.}\label{PredictionsGAMAD}
\end{figure}
...
for(i in 1:total.passes) {
  ...
  #current.gam <- gam(Yield ~ s(Easting,k=30),data=current.pass)
...
\begin{figure}\centering
<<SmoothSwathsMGCV, fig=TRUE, echo=FALSE, width=12, height=10>>=
ggplot(Example1.dat, aes(Easting,Yield)) + 
 #geom_line(linewidth=1, col=cbPalette[1]) + 
 geom_point(size=1, col=cbPalette[1]) + 
 geom_line(linewidth=1, col=cbPalette[2],data=Example1SmoothHarvestPasses.dat) + 
 labs(x='Easting', y='Yield', title = 'Harvest passes from Example 1 with associated functional forms from mgcv') + facet_wrap(~Pass)
@
...
To summarize the magnitude of the problem we are faced with as we desire to undertake a functional data analysis of strip trials, consider Figure \ref{SmoothSwathsMGCV}. This duplicates the harvest passes as shown at the start of this chapter (\ref{RawSwaths}), with functional forms found using the default smoothing parameters for the \texttt{mgcv} library. While for some harvest passes the functional forms are over-smoothed (by visual inspection), as a full data set the defaults for \texttt{mgcv} are sufficient for functional data analysis as described in following chapters. More precisely, unless otherwise noted, we will be using \texttt{mgcv} to find functional forms for harvest pass data.

In Figure \ref{SmoothSwathsMGCV}, the approximating functions are found using \texttt{mgcv} with the default maximim degrees of freedom for the smoother, \texttt{k=9}. For comparison, we now consider finding the optimal \texttt{k} for each strip, individually. To start, we iterate over each of the 24 harvest passes in the first example, fitting \texttt{mgcv gam} to each strip for a range of degrees of freedom (\texttt{k}). We would then inspect each curve to attempt to determine an optimal \texttt{k} value based on the curve of the generalized cross-validation score relative to \texttt{k}. 

For computational speed comparison, we also fit the individual curves from Figure \ref{SmoothSwathsMGCV} using \texttt{brms}. As we see in Figure \ref{SmoothSwathsBRM}, the default settings tended to provide similar looking functional forms as \texttt{mgcv} but took nearly 14 minutes to compute, compared to less than a second for the same number of functional data curves fit using \texttt{mgcv} when computed using a 2022 MacBook Pro with an Apple M2 chip and 8 GB memory.

<<SmoothHarvestStripsExample1BRM, echo=FALSE>>=
if(!file.exists('HarvestPassesBRM.Rda')) {
  StartTimeBRM <- Sys.time()
  Example1.basis <- rep(seq(2,398, by=2))
  total.passes <- max(Example1.dat[,'PassNo'])
  Example1SmoothHarvestPassesBRM.dat <- data.frame(
    Yield = rep(0, total.passes*length(Example1.basis)),
    Easting = rep(Example1.basis, total.passes),
    PassNo = rep(1:total.passes, each=length(Example1.basis)),
    Pair = rep(0, total.passes*length(Example1.basis)),
  ...
  for(i in 1:total.passes) {
    current.pass <- Example1.dat[Example1.dat[,'PassNo']==i,]
    current.brm <- brm(Yield ~ s(Easting),data=current.pass)
    current.smoothed <- predict(current.brm, data.frame(Easting = Example1.basis))[,1]
    Example1SmoothHarvestPassesBRM.dat[,'Yield'][Example1SmoothHarvestPassesBRM.dat[,'PassNo']==i] <- current.smoothed
    Example1SmoothHarvestPassesBRM.dat[,'Pair'][Example1SmoothHarvestPassesBRM.dat[,'PassNo']==i] <- current.pass[,'Pair'][1]
    Example1SmoothHarvestPassesBRM.dat[,'Product'][Example1SmoothHarvestPassesBRM.dat[,'PassNo']==i] <-   as.character(current.pass[,'Product'][1])
    Example1SmoothHarvestPassesBRM.dat[,'Northing'][Example1SmoothHarvestPassesBRM.dat[,'PassNo']==i] <- mean(current.pass[,'Northing'])
  }
  EndTimeBRM <- Sys.time()
  Example1SmoothHarvestPassesBRM.dat[,'Pass'] <- as.factor(Example1SmoothHarvestPassesBRM.dat[,'PassNo'])
  Example1SmoothHarvestPassesBRM.dat[,'Product'] <- as.factor(Example1SmoothHarvestPassesBRM.dat[,'Product'])

  Example1SmoothHarvestStripsBRM.dat <- aggregate(Yield ~ Easting + Product + Pair,FUN=mean, data=Example1SmoothHarvestPassesBRM.dat)
  save(StartTimeBRM, EndTimeBRM, Example1SmoothHarvestPassesBRM.dat,Example1SmoothHarvestStripsBRM.dat,file='HarvestPassesBRM.Rda')
} else {
  load(file = 'HarvestPassesBRM.Rda')
}
@

\begin{figure}\centering
<<SmoothSwathsBRM, fig=TRUE, echo=FALSE, width=12, height=10>>=
ggplot(Example1.dat, aes(Easting,Yield)) + 
 #geom_line(linewidth=1, col=cbPalette[1]) + 
 geom_point(size=1, col=cbPalette[1]) + 
 geom_line(linewidth=1, col=cbPalette[2],data=Example1SmoothHarvestPassesBRM.dat) + 
 labs(x='Easting', y='Yield', title = 'Harvest passes from Example 1 with associated functional forms from \texttt{brm}') + facet_wrap(~Pass)
@
...
In Figure \ref{IndividualGCV} we plot both the ordinary cross-validations scores (OCV) and the generalized cross validation scores (GCV) for individual harvest passes. The scores are nearly identical, so we'll focus on the GCV, which will in general be easier to compute using \texttt{mgcv gam}. For most passes the GCV score decreases with increasing \texttt{k}, but in many cases it is not clear that there is an optimal \texttt{k}, while the default value of \texttt{k=9} would be sufficient for many harvest passes. Using the default \texttt{k} value for \texttt{mgcv gam} simplifies the analysis since the fitting of the harvest passes can be automated, while fitting passes with individual \texttt{k} values would require more extensive data manipulation. 

A more work intensive approach would be to fit a single pass at a time, slowly increasing \texttt{k} values and using the \texttt{gam.check} function to determine if an appropriate \texttt{k} value had been supplied. The output from \texttt{gam.check} includes a test of the maximum degrees of freedom versus the effective degrees of freedom, and an associated $p$-value. We would fit with increasing \texttt{k} values and running \texttt{gam.check} for each fit until an appropriate \texttt{k} is found. This process is illustrated in Table \ref{gam.check.tbl} which was generated by repeatedly calling This table was generated by repeatedly calling \texttt{current.gam <- gam(Yield ~ s(Easting,k=...),data=SinglePass.dat)} and \texttt{gam.check(current.gam)}.
This process may be practical if only one data set is to be analyzed, but we hope our proposed method would be used to analyze many strip trials, where fitting individual passes is impractical. By trial and error, the large \texttt{k} values suggested by \texttt{gam.check} produce functional forms for strips that are too wiggly (results not shown).

<<IndividualCV, echo=FALSE,include=FALSE>>=
if(!file.exists('IndividualCV.Rda')) {
   K <- c()
   Pass <- c()
   CV <- c()
   Method <- c()
   ...
   K2 <- c()
   Pass2 <- c()
   for(pass in unique(Example1.dat[,'PassNo'])) {
      current.pass <- Example1.dat[Example1.dat[,'PassNo']==pass,]
      for(k in 3:20) {
         K <- c(K,k)
         Pass = c(Pass,pass)
         current.gam <- gam(Yield ~ s(Easting,k=k),data=current.pass)
         current.summary<- summary(current.gam)
         CV <- c(CV, current.summary[['sp.criterion']])

         sum <- 0
         for(j in 1:dim(current.pass)[1]) {
            current.d <- current.pass[-j,]
            current.model <- gam(Yield ~ s(Easting, k=k), data=current.d)
            new.dat <- current.pass[j,]
            predicted <- predict(current.model, newdata = new.dat)
            diff <- predicted - new.dat[,'Yield']
            sum  <- sum+(diff)^2
         }
         K <- c(K,k)
         Pass = c(Pass,pass)
         CV <- c(CV, sum/dim(current.pass)[1])
         Method <- c(Method, 'OCV')
         
         K2 <- c(K2,k)
         F = c(F, current.summary[['s.table']][3])
         Deviance <- c(Deviance, current.gam$deviance)
         Pass2 <- c(Pass2,pass)
         
      }
   }
   IndividualCV.dat <- data.frame(K=K, Pass=Pass, CV=CV, Method = Method)
   IndividualScores.dat <- data.frame(K=K2,F=F,Deviance=Deviance,Pass=Pass2)
   save(IndividualCV.dat,IndividualScores.dat, file = 'IndividualCV.Rda')
} else {
   load(file = 'IndividualCV.Rda')
}
@

\begin{figure}\centering
<<IndividualCV, fig=TRUE, echo=FALSE, width=12, height=10>>=
ggplot(IndividualCV.dat, aes(K,CV)) + 
 geom_line(linewidth=1,aes(color=Method, linetype=Method)) + scale_colour_manual(values=cbPalette) + 
 labs(x='K', y='CV', title = 'Cross-Validation Scores for Individual Passes') + facet_wrap(~Pass, scales='free_y')
@
...
#current.gam <- gam(Yield ~ s(Easting,k=3),data=SinglePass.dat)
#gam.check(current.gam)
@
...
The summarized output for \texttt{gam.check} for several \texttt{mgcv} \texttt{gam} models, each generate with a value from the \texttt{k} column. We plot the results of model with the smallest \texttt{k} associated with a non-significant \textit{p}-value in Figure \ref{BestGamCheckSplines}. We also run \texttt{gam.check} with the results of the \texttt{k=30} model. A portion of the output is reproduced below:
\begin{verbatim}
...
This is a contradictory message. This model did not produce a low \texttt{p-value}, while the \texttt{k-index} is 0.97; close to, but not below 1. The return value of \texttt{gam.check} is a list of 2 elements; the values in Table \ref{gam.check.tbl} were obtained by visually examining the display from calling \texttt{gam.check} in R. This prevents that \texttt{gam.check} from being useful for automating model selection. \texttt{summary} of the value returned by \texttt{mgcv} \texttt{gam} does include a member field \texttt{s.table} which contains a term \texttt{F}. This may be useful for automated selection of \texttt{mgcv} models. As showing in Figure \ref{IndividualF}, in most cases there is a clear maximum F value. Automation of spline model selection for individual curves is beyond the score of this thesis.

<<BestGamCheckModel, echo=FALSE, include=FALSE>>=
best.gam <- gam(Yield ~ s(Easting,k=30),data=SinglePass.dat)
gam.check.best <- gam.check(best.gam)
@

\begin{figure}\centering
<<BestGamCheckSplines, fig=TRUE, echo=FALSE, width=8, height=4>>=
BestGamCheckSplines <- data.frame(X=SinglePass.dat$Easting,
                           Predicted=predict(best.gam)) 
ggplot(aes(X,Predicted),data=BestGamCheckSplines) + 
  geom_line() + 
  scale_colour_manual(values=cbPalette[2]) +
  geom_point(aes(Easting, Yield),size = 1, data=SinglePass.dat, color=cbPalette[1])                        
@
...
\begin{figure}\centering
<<IndividualF, fig=TRUE, echo=FALSE, width=12, height=10>>=
ggplot(IndividualScores.dat, aes(K,F)) + 
 geom_line(linewidth=1) + scale_colour_manual(values=cbPalette) + 
 labs(x='K', y='F', title = 'F Values for Individual Passes') + facet_wrap(~Pass, scales='free_y')
@
\caption{\texttt{summary(...)[['s.table']]}, element 3 (\texttt{F} value) from \texttt{mgcv} fits to individual curves. These data suggest possible methods for automated selection of \texttt{k} }\label{IndividualF}
\end{figure}
...
Example1HarvestPasses.dat[,'Product'] <- factor(Example1HarvestPasses.dat[,'Product'],levels=c('B','E'))
...
Example1HarvestPasses.dat[,'NorthingDeviation'] = aggregate(Northing ~ PassNo, Example1.dat, max, na.rm=TRUE)[,2] - aggregate(Northing ~ Pass,Example1.dat, min, na.rm=TRUE)[,2]
...
ggplot(Example1HarvestPasses.dat, aes(x=Northing, y=Yield, color=Product, fill=Product)) +
   ...
   labs(x='Northing', y='Yield', title = 'Example 1 Harvest Pass Yields')
@
...
We now consider strips as a plot composed of two harvest passes, each planted with the same variety. We can then compute strip yield by taking the arithmetic average of the two harvest passes per strip. These results are show in Table \ref{SummaryExample1Strips}, and plotted in Figure \ref{Example1StripYieldsPlot}
...
ggplot(Example1Strips.dat, aes(x=Northing, y=Yield, color=Product, fill=Product)) +
   geom_bar(stat='identity') + scale_colour_manual(values=cbPalette[pair.colors], name='Variety') +     
   scale_fill_manual(values=cbPalette[pair.colors], name='Variety') +
   labs(x='Northing', y='Yield', title = 'Example 1 Strip Yields')
@
...
\begin{figure}\centering
<<Example1VarietyDifferencesPlot, fig=TRUE, echo=FALSE, width=8, height=3>>=
VarietyDifferences.dat <- data.frame(Difference=Differences, PairNo = 1:6)
VarietyDifferences.dat['PairFactor'] <- factor(VarietyDifferences.dat[,'PairNo'])
ggplot(VarietyDifferences.dat, aes(x=PairFactor, y=Difference, color=PairFactor, fill=PairFactor)) +
   geom_bar(stat='identity') + scale_colour_manual(values=cbPalette, name='Pair Number') +     
   scale_fill_manual(values=cbPalette, name='Pair Number') +
   labs(x='Pair Number', y='Yield', title = 'Example 1 Paired Differences')
@
...
For pointwise trend analysis of functional harvest pass data, our model for strip yields takes the form (adapted from \cite{hastie-1996, wood-2017})

\begin{equation}
y_j = \beta a_j\ \beta theta(x_j) + f(z_k) + \epsilon_i(z_k)
...
To begin a trend analysis of strip means, we consider the data from Figure \ref{Example1HarvestYieldsPlot}, but instead of representing harvest pass means as bars, we use points to emphasis that these data are points to be interpolated by a smoothing curve (Figure \ref{PointEstimates}). Note that while the bar chart in Figure \ref{Example1HarvestYieldsPlot} start at 0, the $y$ axis in Figure \ref{PointEstimates} start at 160. This emphasized the trend. 

We will then smooth yield points with a \texttt{mgcv} generalized additive model. Thiis syntax of the form \texttt{Yield \textasciitilde Product + s(Northing)}, where \texttt{Product} is the parametric portion of the model and \texttt{Northing} is a spatial covariate. \texttt{Product} is the name of the variety data column in the original data; we continue to use that nomenclatur. We plot points associated with harvest passes instead of strips because we expect the trend to account for spatial variability. Paired $t$ tests use pairs to group observations into homogeneous units; this is one way to account for spatial variability.

\begin{figure}\centering
<<PointEstimates, fig=TRUE,echo=FALSE, width=8, height=3>>=
point.colors <- c(pair.colors[2],pair.colors[1])
#Now we add the predicted values to the trend
ggplot(Example1HarvestPasses.dat, aes(x=Northing, y=Yield)) +
  geom_point(aes(color=Product, fill=Product), name='Variety') + scale_colour_manual(values=cbPalette[point.colors],name='Variety') + scale_fill_manual(values=cbPalette[pair.colors], name='Variety')
@
\caption{}
...
textit{toothed} pattern, as each pair of yield observations represents a different variety from the adjacent pair.}
...
The curves in Figure \ref{SubsetAndAnalyze21416RawDataPlot} themselves are not simple lines, since the \texttt{mgcv} model includes `Variety` in the fixed effects component of the model. To visualize just the spatial trend, we used the \texttt{mgcv} models that were fit to the yield data in Figure \ref{SubsetAndAnalyze21416RawDataPlot}, but predict yield from these models using only variety \texttt{E} as the basis for prediction. Since variety \texttt{E} is the reference fixed effect in these models, the resulting curves show only the smoother for yield, or the functional form of the spatial yield trend, as predicted by the \texttt{mgcv} models. We show these results in Figure \ref{SubsetAndAnalyze21416RawDataPlotE}.

\begin{figure}\centering
<<SubsetAndAnalyze21416RawDataPlotE, fig=TRUE,echo=FALSE, width=8, height=3>>=
Example1SmoothHarvestStrip214.dat[,'Product'] <- levels(Example1SmoothHarvestStrip214.dat[,'Product'])[1]
Example1SmoothHarvestStrip216.dat[,'Product'] <- levels(Example1SmoothHarvestStrip216.dat[,'Product'])[1]
Example1SmoothHarvestStrip214.dat[,'Predicted'] <- predict(Smoothed214.gam,newdata = Example1SmoothHarvestStrip214.dat)
Example1SmoothHarvestStrip216.dat[,'Predicted'] <- predict(Smoothed216.gam, newdata = Example1SmoothHarvestStrip216.dat)
Smoothed2146E.dat = rbind(Example1SmoothHarvestStrip214.dat,Example1SmoothHarvestStrip216.dat)
Smoothed2146E.dat[,'Easting'] <- as.factor(Smoothed2146.dat[,'Easting'])
ggplot(Smoothed2146E.dat, aes(Northing, Yield)) + 
geom_point(aes(col=Easting)) + #cbPalette[1]) + 
geom_line(aes(Northing, Predicted, col=Easting),data=Smoothed2146E.dat) +
scale_colour_manual(values=cbPalette) +
labs(x='Northing', title = 'Yield Estimates at Easting 214 and 216, Variety E only')
@
...
Now, we've consider $\delta$ as a random effect, but Schabenberger \cite{schabenberger-2001} reminds of the adage \textit{one modeler's random effect is another modeler's fixed effect}. Specifically, in the case of $\delta$, we capture the spatial correlation structure as a smooth function over points $\mathbf{s}$, that is, $f(\mathbf{s}) = f(r, c)$, where $r$ is a row indicator and $c$ is a column indicator. This lead us to model the spatial structure of yield monitor data as a smoothed function of a generalized additive model (Equation \ref{GAMFormula}),

\begin{equation}
   g(y_{i}) = \mathbf{A_i} \mathbf{\theta} + f(x_2, x_3) + e_{i}
\end{equation}\label{SpatialGAM}

where $f$ is a smooth response (or trend) surface, $\mathbf{A_i}$ is a treatment structure associated with a point specified by row $x_2$ and column $x_3$, $\mathbf{\theta}$ are parameters associated with treatments and estimated from the data, and $f(x_2,x_3)$ is a smoothing function approximating the random field $\delta$. $e_{ij}$ are an uncorrelated white noise process. Again, for these data, $g$ is the identity function, and there is no $f(x_1)$. Here we treat the yield data as a realization of random field with noise, and it can be shown that random fields can be approximated using appropriate polynomials \cite{kent-1989}. We can use generalized additive models (i.e. \texttt{mgcv}) to fit this the random field as a response surface, using syntax of the form \texttt{s(Easting,Northing, k = *)}  We can also use \texttt{lm} with splines, specifically b-splines, using syntax of the form \texttt{bs(Easting, df=*)*bs(Northing, df=*)}

How should we determine the appropriate degree of smoothing? We must show some concern for estimating a response surface that reflects random noise \cite{matheron-1967}. For this, we consider the properties of the semivariogram.
...
Again from Schabenberger \cite{schabenberger-2001} we define the semivariogram of $Z(\mathbf{s})$ as
...
\section{Empirical semivariogram estimators}
 Following Schabenberger \cite{schabenberger-2001}, we write the estimator of the semivariogram as
...
For our purposes, we aren't particularly interested in the sill or range. But the nugget, $\theta_0$, is of interest, as we'll discuss later. 

\begin{figure}\centering
<<TheoreticalSphericalVariogramA, fig=TRUE,echo=FALSE, width=8, height=4>>=
h <- 1:200/100
range = 1
sill = 1
nugget = 1
Variogram <- rep(nugget + sill, length(h))
Variogram[h<range] <- nugget+(sill)*{(3*h[h<range]/2*range)-0.5*(h[h<range]/range)^3}
Var.dat <- data.frame(h = h, Variogram = Variogram)
ggplot(Var.dat, aes(h, Variogram)) + 
geom_line(aes(color = cbPalette[1]), size=2) +
ylim(c(0,2))+
scale_colour_manual(values=cbPalette) +
labs(x='h',y = 'Semivariance', title = 'Theoretical Spherical Variogram') +
...
Ideally, the semivariogram will have a nugget of 0, as points not separated in space should be equal, and as such have no variance. That is, $\gamma(\mathbf{h}) \to 0$ as $\lVert \mathbf{h} \rVert \to 0$. In most data, there is a nugget $\theta_0 \ne 0$ such that $\gamma(\mathbf{h}) \to \theta_0$ as $\lVert \mathbf{h} \rVert \to 0$. Schabenberger \cite{schabenberger-2001} argues that the nugget is composes of two processes, a measurement error $\sigma^2_e$ that results from errors that might arise from repeated measurements at a single point in space, and a variance component derived from processes that operate on a smaller spatial scale than the observed lags ($\mathbf{h}$) in the data set; this process results in a variance component $\sigma^2_{\eta}$; such that $\theta_0 = \sigma^2_e + \sigma^2_{\eta}$. For our purposes, we will ignore $\sigma^2_{\eta}$, and assume that

\begin{equation}
\theta_0 = \sigma^2_e
\end{equation}

That is, the nugget in our semivariograms is a measure of the effects of $e_i$ from Equation \ref{SpatialGAM}. We seek to partition semivarigrams into two components; the part described by $f(\mathbf{s} = f(x_2, x_3)$ and one that is solely comprised of $e_i$. A semivariogram with a nugget of 0 is show in Figure \ref{TheoreticalSphericalVariogramB}.

\begin{figure}\centering
<<TheoreticalSphericalVariogramB, fig=TRUE,echo=FALSE, width=8, height=4>>=
h <- 1:200/100
range = 1
sill = 1
nugget = 0
...
\begin{enumerate}
  \item Select a range of \texttt{k} values to consider. This is largely done by trial an error. We select a sufficiently large range (i.e. 30-360 for \texttt{mgcv} models and 3-36 for \texttt{lm} models), then examine the semivariograms and adjust \texttt{k} or \texttt{df} values to provide tighter coverage of the critical values. 
...
#for gam models, I find that k in the range of 30-360 are reasonable, 
#for lm models, each call to bs in the range of 3-36
SpatialGamKs = (1:12)*30
SpatialLMBs = (1:12)*3
...
ggtitle('Response surface using lm estimation') +
...
ggtitle('Response surface using gam estimation') +
labs(colour = 'Yield', x='Easting', y='Northing') + facet_wrap(~ K)
@
...
Figure \ref{Example1LMPredictions} show the expected response surface of a field planted with only variety E based on linear models with b-splines fit to Example 1, with varying degrees of freedom associated with the spline model. At small degrees of freedom, we see little of the field structure observable in Figure \ref{Example1Maps}. As degrees of freedom increases, we see more of the yield structure, with low yield zones apparent at \texttt{df = 9} and  \texttt{df=12}. As \texttt{df} increases we see more detail, included banded structures where harvest passes may be misaligned. However, at \texttt{df=25} we see an abrupt change in the contours of the response surface, with prominent bands where one variety is planted; these bands grossly underestimate yield. At \texttt{df=30} these bands disappear, but are replaced with less prominent bands of higher than expected predicted yields. This series of response surfaces suggests against using \texttt{lm} with b-splines to generate response surfaces from yield monitor data.
...
@
\caption{Selection criteria for response surfaces fit as in Figure \ref{Example1VariogramsGAMPlot}. In the upper left subgraph, we see that adjusted $R^2$ increases dramatically with small \texttt{k}, then increases steadily with increasing model complexity. Using the \textit{elbow} method, we might prefer a model with \texttt{k=90}. The upper right subgraph is nearly a mirror image of the upper left subgraph; here we see that the generalized cross validation score decreases with model complexity, and application of the \textit{elbow} method would similarly lead us to choose a model with \texttt{k=90}. The lower left sub graph suggests a model of \texttt{k=120} as the most complex model without a significant nugget effect, while the lower right subgraph suggests that models with \texttt{k>30} retain some spatial correlation in the residuals.}\label{Example1ModelSelectionGAM}
\end{figure}
...
<<Example1ExtractCoefficients, include=FALSE, echo=FALSE>>=
Coef <- c()
Source <- c()
K <- c()
UB <- c()
...
\begin{figure}\centering
<<Example1CoefficientsLM, fig=TRUE,echo=FALSE, width=8, height=3>>=
...
\begin{figure}\centering
<<Example1CoefficientsGAM, fig=TRUE,echo=FALSE, width=8, height=3>>=
 ggplot(Plot1[Plot1['Source'] == 'MGCV',], aes(K,Coef)) + 
     geom_ribbon(aes(ymin = LB, ymax = UB),col=cbPalette[1],fill=cbPalette[2]) +
         geom_point(size=2) + 
         geom_line(linewidth=1) + 
         scale_colour_manual(values=cbPalette) + 
         labs(title='Variety B effect, mgcv, Example 1', x ='Smoothing Parameter', y = 'Product Effect Estimate')
@
...
<<Example2FullExtractCoefficients, include=FALSE, echo=FALSE>>=
Coef <- c()
Source <- c()
K = c()
UB <- c()
...
\begin{figure}\centering
<<Example2FullCoefficients, fig=TRUE,echo=FALSE, width=8, height=3>>=
...
ggtitle('Response surface using brm estimation') +
labs(colour = 'Yield', x='Easting', y='Northing') + facet_wrap(~ K)
@
...
<<Example1ExtractCoefficientsBRM, include=FALSE, echo=FALSE>>=
Coef <- c()
LB <- c()
UB <- c()
K <- c()
...
\begin{figure}\centering
<<Example1BRMCoefficients, fig=TRUE,echo=FALSE, width=8, height=3>>=
ggplot(PlotBRM, aes(K,Coef)) + 
      geom_ribbon(aes(ymin = LB, ymax = UB),col=cbPalette[1],fill=cbPalette[2]) +
      geom_point(size=2) + 
      geom_line(linewidth=1) + 
      scale_colour_manual(values=cbPalette) + 
      labs(title='Variety B effect, brm, Example 1', x ='Smoothing Parameter', y = 'Product Effect Estimate')
@
...
<<Example2SubExtractCoefficients, include=FALSE, echo=FALSE>>=
Coef <- c()
Source <- c()
K = c()
UB <- c()
...
\begin{figure}\centering
<<Example2SubCoefficients, fig=TRUE,echo=FALSE, width=8, height=3>>=
ggplot(Plot2, aes(K,Coef)) + 
      geom_ribbon(aes(ymin = LB, ymax = UB),col=cbPalette[1],fill=cbPalette[2]) +
      geom_point(size=2) + 
      geom_line(linewidth=1) + 
...
We spend only a little discussion on the analysis of the subset of Example 2. From Figures \ref{Example2SubGAMPredictions}, \ref{Example2SubVariogramsGAMPlot} and \ref{Example2SubModelSelectionGAM}, the analysis of the subset of Example 2 is not greatly different from the analysis of the full data set. However, it is worth noting that the estimated effect due to fungicide spraying in Figure \ref{Example2SubCoefficients} is consistently negative for \texttt{k>0}, and for the best models (\texttt{k=90}) the effect of fungicide spray was a loss of roughly 3 bu/acre.
...
<<EndTiming, eval=FALSE, include=FALSE, echo=FALSE>>=
#EndTypesetting <- Sys.time()
#Rprof(NULL)
#summaryRprof()$by.total[20,]
#                                       total.time total.pct self.time self.pct
#"brm"                                    11581.94     86.89      0.00     0.00
#"gam"                                      643.52      4.83     17.08     0.13
#> 11581.94/643.52
#[1] 17.99779
@
