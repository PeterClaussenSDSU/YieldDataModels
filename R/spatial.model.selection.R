spatial.model.selection <- function(data, range=c(5,100), models=8, method='gam',plot=FALSE,LISA=FALSE) {
  ...
  if(method %in% c('gam','tp','te','re')) {
    null.model <- gam(Yield ~ 1,data=data[,c('Longitude','Latitude','Yield')])
  } else {
  ...
  for(i in 1:length(k.vec)) {
    tmp.dat <- data[,c('Longitude','Latitude','Yield')]
    if(method=='idw') {
      ...
      if(method=='gam') {
        ...
        Models[[i]] <- lm(Yield ~ bs(Longitude,df=k.vec[i])*bs(Latitude,df=k.vec[i]), data=tmp.dat)
      } else if(method=='lm'){
...
#  Selection$Score <- c(Selection$Score,Slopes.dat$Slope)
  ...
  ret = list(Variograms=Variograms,
              Residuals=Residuals,
